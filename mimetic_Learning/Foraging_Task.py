...
def main(learning_algorithm):
    if learning_algorithm == "neural":
        rob = Pyroborobo.create(
            "config/ForagingTask.properties",
            controller_class=NeuralLearner,
    ...
    if c.SAVE_FILE:
        if c.LEARNT_BEHAVIOUR_PROPAGATION :
            DataHandler.save_data(f"{c.SAVE_FILE}_{c.DISCRETISE_RATIO}_with_propagation")
        else :
            DataHandler.save_data(f"{c.SAVE_FILE}_{c.DISCRETISE_RATIO}")


if __name__ == "__main__":
    main(c.LEARNING_ALGORITHM)
