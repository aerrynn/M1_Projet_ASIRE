...
import numpy as np
from supervisedLearning import perceptronSL
...
class hit_ee_v4():
    ...
    def __init__(self, selfRobot, tR, mD, setVerbose = False):
        
        # Initialization
        global fitness, verbose, selfC, transferRate, maturationDelay, isFirstIteration
    ...
    def broadcast(self):
        for i in range (selfC.nb_sensors):
            ...
            nbBehavioursToSend = 1 #tooooooooooooooooooooooooooooo set in parameters
            behavioursToSend = np.random.choice(range(0, len(selfC.tabMyBehaviors)), nbBehavioursToSend, False)
            nbElemToReplace = int(len(selfC.genome) * transferRate)
            elemToReplace = np.random.choice(range(0, len(selfC.genome)), nbElemToReplace, False)
